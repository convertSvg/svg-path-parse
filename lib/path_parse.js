...
const { toDecimal, getCircleCenter } = require('./utils')
      ...
      if (index > 0) {
        ...
        switch (code) {
          ...
          case 0x61/* a 7 params  drawing arc */:
            ...
            // circle
            if (item[0] === item[1]) {
              // (item[0] item[1]) item[3] item[4] item[5] item[6] => rx=ry large-arc-flag sweep-flag x y
              item = getCircleCenter(x, y, item[5], item[6], item[0], item[3], item[4])
            } else { // ellipse
          ...
          case 0x41/* A 7 params  drawing arc */:
            prevPos = [item[5], item[6]]
            if (item[0] === item[1]) {
              // item[0] item[3] item[4] item[5] item[6] => rx=ry large-arc-flag sweep-flag x y
              item = getCircleCenter(x, y, item[5], item[6], item[0], item[3], item[4])
            } else { // ellipse
